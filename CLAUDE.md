...
- **`Timestamp` GUANO avec le `T`, `wamd` avec un espace — les deux formats diffèrent réellement.** Vérifié octet par octet sur `test-data/real_process_teensy/…_000_Processed.wav` : Kaleidoscope écrit `2026-05-07T21:11:06+02:00` dans le GUANO et `2026-05-07 21:11:06+02:00` dans le wamd. chiro mettait un espace dans les deux jusqu'à ce que l'audit de mutation le trouve — en lisant un fichier de référence présent dans le dépôt qu'aucun test n'ouvrait. Ne pas « harmoniser » les deux fonctions de formatage : leur divergence est le comportement attendu.
...
- **`cleanOrphanStagingDirs`** (appelé au **constat**, jamais au run où il figerait l'UI) : liveness PID **plus** garde d'âge 24 h (les PID sont recyclés — sinon un `kill -9` peut laisser un dossier caché de plusieurs Go), `lstat` + `isDirectory()` (ne jamais suivre un lien), suppression **fichier par fichier** des seuls noms attendus puis `rmdir` — **jamais `recursive: true`**. Un `ENOTEMPTY` fait renoncer plutôt qu'écraser du contenu inconnu. La règle vaut pour ce nettoyage-là, qui opère sur des chemins étrangers ; le staging du run **en cours**, fabriqué par le module dans la même fonction, est bien détruit par `rm(recursive, force)`.
- **`ArchiveRunner<TOk>` est générique, et ce n'est pas cosmétique.** Unionner les succès des deux flux obligeait chacun à traiter celui de l'autre — quatre branches inatteignables, dont deux fabriquant un code `wrong-behavior-wired:*` que rien ne pouvait émettre. Un troisième flux en aurait produit neuf. Paramétré, brancher le runner de sauvegarde sur la route de dépôt est une **erreur de compilation** (vérifié en câblant volontairement faux). La contrainte `` `${string}-ok` `` est load-bearing : elle garde les succès disjoints de `aborted`/`error`, donc le narrowing sur `kind` continue de fonctionner.
- **`durable` est un booléen obligatoire, jamais optionnel.** Il garde une décision destructive (le futur « uploader puis supprimer `processed/` ») : `undefined` y rendrait `if (r.durable)` et `if (r.durable !== false)` opposés, sans que le type tranche. Il était absent de `BackupOkResult` alors que le zip de sauvegarde est justement l'artefact concerné. L'ajouter a imposé un **`schema_version: 5`** pour `vigie-archive` (v3 retiré) — la règle « une forme d'événement = un numéro » du `types.ts` ne se contourne pas.
- **Le staging n'est jamais adopté, et le commit ne publie que ce que le run a écrit.** Son nom ne dépend que de la série et du PID : il est donc identique d'une tentative à l'autre dans le même processus, et `cleanOrphanStagingDirs` exempte justement les PID vivants — dont le nôtre. D'où deux verrous : `mkdir` **sans** `recursive` (sur `EEXIST` on détruit le reliquat et on recrée — échouer casserait un réessai légitime), et un `readdir` du staging comparé aux volumes réellement produits **avant** le commit. Sans eux, une série annoncée à 2 fichiers pouvait en publier 4, dont un `<série>_part9.zip` valide issu d'un run raté — indiscernable d'un vrai sur le portail. Bug trouvé par deux revues indépendantes et reproduit ; les deux verrous sont vérifiés par mutation.
...
## Archivage en ligne Scaleway Glacier (Phase 10) — pièges à connaître

`src/lib/offsite/` + `src/screens/offsite/` (ADR + détails complets dans `docs/architecture.md` § « Module `lib/offsite` »). Capacité **optionnelle** (cf. ADR § « Les trois statuts possibles d'un binaire externe ») : envoie `archived/<préfixe>_YYYYMMDD.zip` — le gros zip de sauvegarde, jamais la série de dépôt — vers un bucket Scaleway en classe `GLACIER` via `rclone`, jamais un prérequis pour le reste de l'app. Écrans `O-*`, entrée de menu conditionnelle en 5ᵉ position **et** touche `A` en fin de `archive/ResultScreen.tsx`, deux routes vers le même constat.

- **`resolveOffsiteAvailability` exige `rclone` détecté (sonde synchrone dans `index.tsx`, avant `render()` — même pattern que `detectSox`) _et_ `~/.chiro/settings.json` valide (clé `coffre` : `remote`/`bucket`/`prefix`)**. L'un ou l'autre manque → silence total, aucun message, six entrées de menu inchangées.
- **Détection « déjà en ligne » = HEAD distant systématique (`remoteStat`), jamais une relecture de `sessions.jsonl`** : la clé du journal n'est pas unique entre deux études traitées le même jour (`buildArchiveName` retombe sur `"processed"`), donc une détection par journal aurait pu annoncer « déjà archivée » pour un zip jamais envoyé — silencieux, indiscernable d'un succès, le pire échec possible pour une fonction de sauvegarde.
- **Contrat de progression : `totalBytes` et `eta` de rclone sont ignorés partout, jamais lus.** Mesurés faux sur trace réelle (`test-data/rclone-stats/`) : `totalBytes` vaut `0` sur les deux premiers ticks d'un transfert nominal et ne vaut jamais la taille réelle de l'objet une fois un retry survenu ; `bytes / totalBytes` vaut exactement `1,0` sur le tick final d'un transfert qui a **échoué**. Le dénominateur est la taille locale (déjà connue, chiro a `stat`é le zip pour le choisir), le numérateur `transferring[0]?.bytes ?? stats.bytes` clampé monotone.
- **SIGINT, pas SIGKILL, en premier sur abandon** : `kill -INT` sur rclone sort en code `130`, ~0,16 s, sans tick final — l'abandon se détecte par le drapeau d'intention de chiro, jamais par la forme de sortie. Escalade automatique bornée (SIGINT → resend SIGINT à 5 s → SIGKILL à 15 s) : un SIGKILL immédiat laisserait les parts multipart en vol, facturées. Vérifié à la main le 17/08/2026 : après Ctrl+C en plein transfert, `rclone backend list-multipart-uploads` revient vide. **Course non négligée** : rclone peut sortir `0` dans la fenêtre entre l'abandon et l'arrivée du signal — le code de sortie est donc consulté même après une demande d'abandon, `0` reste un succès.
- **`caffeinate -i -s` préfixe rclone, n'est jamais spawné en process séparé** (`buildRcloneCommand`, darwin only) : un process séparé survivrait à un `kill -9` de chiro et laisserait le Mac ne plus jamais dormir, invisiblement. `-s` est inopérant sur batterie (le manuel `caffeinate` le dit) — d'où `pmset -g batt` lu à la confirmation, seule protection restante sur batterie.
- **`useOffsiteRun` est distinct de `useArchiveRun`, volontairement.** `useArchiveRun` est déjà générique sur `TOk` — la forme du succès n'a jamais été l'obstacle. Les vrais obstacles : `entries`/`totalBytes` inscrits dans les options **et** la signature du runner, `VolumeProgressEvent` figé dans `onProgress`, `resolveTargetName` qui veut dire « nom à créer » là où l'offsite a besoin de « fichier qui existe déjà ». C'est la **troisième** copie du couple `AbortController`/`runningRef`/latch de démarrage/`cancelled` — Règle de Trois notée, extraction **délibérément pas faite** (huit lignes de refs, trois contextes qui diffèrent partout ailleurs).
- **Exception documentée à la 3ᵉ condition de l'ADR** : l'écran « taille différente » remplace réellement l'objet distant sur `Entrée` (pas de versioning sur le bucket). Bornée au seul objet distant de même nom, jamais `processed/`, jamais `archived/` — défendable parce qu'un transfert interrompu ne laisse jamais aucun objet visible (le multipart n'est jamais commité), donc un objet présent de taille différente est une version antérieure complète, jamais un envoi tronqué.

## Pièges connus

- **APFS case-insensitive** : `foo.wav` et `FOO.WAV` collisionnent sur macOS. `applyRenames` vérifie via `fs.access` juste avant chaque rename — c'est là que la collision est attrapée, pas dans `planRenames` (dont le set de cibles est volontairement sensible à la casse : deux sources qui ne diffèrent que par la casse n'existent que sur un système sensible à la casse, où leurs cibles coexistent aussi).
...
- **`bsdtar` doit être installé dans TOUT job Linux qui lance `pnpm check`** (`sudo apt-get install -y libarchive-tools`). Les tests d'archive valident chaque zip produit contre trois extracteurs indépendants et **échouent durement** si l'un manque en CI (un skip silencieux serait une couverture fantôme). `unzip` et `python3` sont fournis par les images GitHub, `bsdtar` non — sur Linux. Trois jobs lancent `pnpm check` aujourd'hui : la matrice de `ci.yml`, et les deux `build-*` de `release.yml` (celui de macOS a `bsdtar` en standard). Le piège s'est déclenché deux fois : le message d'échec du test indique désormais la commande d'installation.
...
- **`rclone lsjson --stat` sur un objet absent sort en code 0**, pas en code d'erreur : `{"IsDir": true, "Size": -1}`, un pseudo-répertoire. **Le discriminant est `IsDir`, jamais le code de sortie seul** — lire `Size` sur tout exit 0 conclurait « présent, taille -1 », la lecture la plus dangereuse possible puisqu'elle est sur le chemin nominal (le tout premier archivage d'une étude). Code 3 = bucket introuvable, code 1 = remote/config inconnu. Mesuré sur le vrai bucket le 17/08/2026, rclone v1.75.0. `Tier` est lisible sans `-M`.
- **Le contrat de stats rclone (`--use-json-log --stats`) est contre-intuitif, et interdiction formelle de lire `totalBytes`/`eta`.** `totalBytes` vaut `0` sur les deux premiers ticks d'un transfert nominal, et ne vaut jamais la taille réelle une fois un retry survenu (mesuré : 240 447 488 → 271 179 776 → 92 196 864 pour un fichier de 209 715 200 octets). Pire, `bytes / totalBytes` vaut exactement `1,0` sur le tick final d'un transfert qui a **échoué**. Fixtures réelles committées dans `test-data/rclone-stats/` (capturées le 17/08/2026, README dédié des invariants) — `lib/offsite/parseRcloneStats.ts` s'y rejoue octet pour octet et à des tailles de chunk adverses. Ne jamais réintroduire une lecture de `totalBytes`/`eta` dans ce module.
- **`caffeinate` doit préfixer la commande rclone, jamais être spawné en process séparé** (`buildRcloneCommand.ts`, darwin only) : un process séparé survit à un `kill -9` de chiro et laisse le Mac ne plus jamais dormir, invisiblement, jusqu'au redémarrage. Et **`caffeinate -s` est inopérant sur batterie** (« valid only when system is running on AC power », dixit son propre manuel) — `readPowerState` (`pmset -g batt`) existe pour ça, pas par redondance : sur batterie, seule la phrase de l'écran de confirmation protège.
- **`FORCE_COLOR` doit être absent de l'environnement de test, sinon huit tests d'écran rougissent.** Ink colorise via chalk, dont le niveau se décide depuis l'environnement ambiant — un dev qui exporte `FORCE_COLOR` (courant, certains CLI le font pour garder leurs couleurs à travers un pipe) injecte des séquences ANSI dans les chaînes que les tests d'écran comparent brutes (`toContain`), pour un diff qui ressemble à une vraie régression sans en être une. Épinglé dans `vitest.config.ts` (`env: { FORCE_COLOR: "0" }`) depuis la Phase 10 — la suite dépend désormais du dépôt, pas du shell de qui la lance.
