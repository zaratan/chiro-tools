...
rmSync(SOX_DIR, { recursive: true, force: true });
...
for (const spec of SOURCES) {
  ...
  const outDir = path.join(SOX_DIR, sourceBase);
  mkdirSync(outDir, { recursive: true });

  const segmentSeconds =
    spec.mode === "expand-10x" ? CHUNK_SECONDS / 10 : CHUNK_SECONDS;
  const outputBase = path.join(outDir, "raw_.wav");
  ...
  if (result.status !== 0) {
    console.error(`  ✗ ${spec.key}: sox exit ${result.status}`);
...
writeFileSync(
  path.join(SOX_DIR, "manifest.json"),
